...
int negotiate(struct s2n_connection *conn, int fd)
{
    ...
    if (conn->psk_params.chosen_psk) {
        printf("Chosen PSK wire index: %d\n", conn->psk_params.chosen_psk_wire_index);
        printf("Chosen PSK type: %s\n", (conn->psk_params.chosen_psk->type) ? "S2N_PSK_TYPE_EXTERNAL" : "S2N_PSK_TYPE_RESUMPTION");
        printf("Chosen PSK identity size: %d\n", conn->psk_params.chosen_psk->identity.size);
        printf("Chosen PSK identity data: %s\n", conn->psk_params.chosen_psk->identity.data);
        printf("Chosen PSK obfuscated ticket age: %d\n", conn->psk_params.chosen_psk->obfuscated_ticket_age);
    }
