...
int main(int argc, char **argv) {
    BEGIN_TEST();
    {
        ...
        for (int i = 0; i < s2n_supported_curves_list_len; i++) {
            ...
            evp_params.negotiated_curve = s2n_supported_curves_list[i];
    ...
    }
    {
        ...
        for (int i = 0; i < s2n_supported_curves_list_len; i++) {
            ...
            /* Server generates a key */
            server_params.negotiated_curve = s2n_supported_curves_list[i];
            EXPECT_SUCCESS(s2n_ecc_evp_generate_ephemeral_key(&server_params));

            /* Client generates a key */
            client_params.negotiated_curve = s2n_supported_curves_list[i];
    ...
    }
    {
        ...
        for (int i = 0; i < s2n_supported_curves_list_len; i++) {
            for (int j = 0; j < s2n_supported_curves_list_len; j++) {
                struct s2n_ecc_evp_params server_params = {0};
                struct s2n_ecc_evp_params client_params = {0};
                struct s2n_blob server_shared = {0};
                struct s2n_blob client_shared = {0};
                ...
                /* Server generates a key */
                server_params.negotiated_curve = s2n_supported_curves_list[j];
    ...
    }
    {
        ...
        for (int i = 0; i < s2n_supported_curves_list_len; i++) {
            ...
            test_params.negotiated_curve = s2n_supported_curves_list[i];
            ...
            EXPECT_EQUAL(avail, s2n_supported_curves_list[i]->share_size);

            /* Verify output starts with the known legacy form for curves secp256r1
             * and secp384r1*/
            if (s2n_supported_curves_list[i]->iana_id == TLS_EC_CURVE_SECP_256_R1 ||
                s2n_supported_curves_list[i]->iana_id == TLS_EC_CURVE_SECP_384_R1) {
    ...
    }
    {
        ...
        for (int i = 0; i < s2n_supported_curves_list_len; i++) {
            ...
            /* Server generates a key for a given curve */
            write_params.negotiated_curve = s2n_supported_curves_list[i];
            ...
            /* Read point back in */
            EXPECT_SUCCESS(
                s2n_ecc_evp_read_params_point(&wire, s2n_supported_curves_list[i]->share_size, &point_blob));

            /* Check that the blob looks generally correct. */
            EXPECT_EQUAL(point_blob.size, s2n_supported_curves_list[i]->share_size);
    ...
    }
    {
        ...
        for (int i = 0; i < s2n_supported_curves_list_len; i++) {
            ...
            write_params.negotiated_curve = s2n_supported_curves_list[i];
            read_params.negotiated_curve = s2n_supported_curves_list[i];
